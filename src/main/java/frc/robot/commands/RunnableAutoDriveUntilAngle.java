...
public class RunnableAutoDriveUntilAngle implements Runnable {
    ...
    public RunnableAutoDriveUntilAngle(DriveSubsystem m_DriveSubsystem, BoolSupplierDriveUntilAngle boolSupplier) {
    ...
    public void run() {
        ...
        if ((m_DriveSubsystem.getAngles()[angleValue] < -11) || (hasReachedStation)) {
            ...
            if (m_DriveSubsystem.getAngles()[angleValue] > previousAngle + 0.07 && m_DriveSubsystem.getAngles()[1] >  -10/*MathUtil.applyDeadband(m_DriveSubsystem.getAngles()[1], 3) == 0*/) {
                //stopped
            ...
            m_DriveSubsystem.drive(0.20, 0, 0, true);
        }
        // else {
