...
public class AutoPickForTwoCubesBlueSide extends SequentialCommandGroup {
  /** Creates a new Autonomous Program. */

  public AutoPickForTwoCubesBlueSide(DriveSubsystem driveSubsystem, ArmWithPIDAndMotionProfile m_arm, GrabberWithPIDAndMotionProfile m_grabber) {
      ...
      Commands.sequence(
        Commands.parallel(
          ...
          Commands.sequence(
            ...
            Commands.parallel(
              new TrajectoryCommand(driveSubsystem, TrajectoryPaths.GoForwardToTurn())
      /*       Commands.sequence(
              new WaitCommand(2),
        ...
        Commands.parallel(
          ...
          Commands.sequence(
            ...
            new TrajectoryCommand(driveSubsystem, TrajectoryPaths.GoBackwardsTowardsBlock())
          )
        ),
        Commands.parallel(
          ...
          Commands.sequence(
            new WaitCommand(1),
            new TrajectoryCommand(driveSubsystem, TrajectoryPaths.GoForwardToTurnOnWayBack()),
            new TurnToAngleProfiled(-160, driveSubsystem).withTimeout(0.5),
            new TurnToAngleProfiled(-0, driveSubsystem).withTimeout(3),
            new TrajectoryCommand(driveSubsystem, TrajectoryPaths.GoBackwardsToDropBlock())
          )
        ),
        new GrabberWithPIDProfiledCommand(m_grabber, Math.toRadians(100)).withTimeout(1)
